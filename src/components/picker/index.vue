  ...
  methods: {
    ...
    setLinkLayer2() {
      ...
      if (typeof defaultIndex === "number") {
        ...
        this.result[1] = this.column2[defaultIndex[1]];
      }
    },
    setLinkLayer3() {
      const { defaultIndex } = this;
    ...
    change(index, res) {
      ...
      this.$emit("change", { selected: this.selected, result: this.result });
    },
    change1(res) {
      if (res) {
        this.change(0, res);
        if (this.layer > 1) {
          this.dIndex2 = 0;
          this.change(1, res.children?.length && res.children[0]);
          this.changeLink("column2", res);
        }
      }
    },
    change2(res) {
      console.log("change2==", res);
